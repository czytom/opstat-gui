class Network 
  ...
  def self.all_interfaces_charts(options)
    charts = []
    Network.where( {:timestamp => { :$gt => options[:start]}, :host_id => options[:host_id], :plugin_id => options[:plugin_id] }).order(:timetamp).all.group_by{|u| u.interface}.each_pair do |interface, values|
  ...
  def self.interface_chart(interface, values)
    ...
    values.each do |value|
      ...
      chart_data[:graph_data] << {
        ...
        "bytes_receive_per_sec" => ((value[:bytes_receive] - prev[:bytes_receive])/time_diff),
        "bytes_transmit_per_sec" => ((value[:bytes_transmit] - prev[:bytes_transmit])/time_diff)
      }
      prev = value 
