class OracleTablespacesSizes
  ...
  def self.all_tablespaces_charts(options)
    charts = []
    OracleTablespacesSizes.where( {:timestamp => { :$gt => options[:start]}, :host_id => options[:host_id], :plugin_id => options[:plugin_id] }).fields(:timestamp, :used, :free, :name).all.group_by{|u| u.name}.each_pair do |tablespace, values|
